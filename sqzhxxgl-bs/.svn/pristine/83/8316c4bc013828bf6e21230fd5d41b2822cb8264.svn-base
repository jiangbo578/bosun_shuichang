...
<body>
    ...
    <li class="active">社区民防应急箱管理记录</li><!--
    <li /tsjbzx/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong> 社区民防应急箱管理记录
			...
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="sqmfyjx:add">
						<a  style="margin-bottom: 5px" href="/sqmfyjx/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="sqmfyjx:update">
											...
											<a class="btn " href="/sqmfyjx/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="sqmfyjx:delete">
											<a class="btn  b_delete_zcdygk" href="/sqmfyjx/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/sqmfyjx/item/${businessForm.id }"><i class=" icon-folder-open"></i>名单</a>
										<shiro:hasPermission name="sqmfyjx:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			                              <a class="btn b_submit_xfqcwh" href="/sqmfyjx/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
