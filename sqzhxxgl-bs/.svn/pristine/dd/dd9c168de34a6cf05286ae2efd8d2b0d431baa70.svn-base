...
<body>
  ...
  <ul class="tabbtn">
		...
		<ul class="tabbtn">
		<li  >
				<span style="width: 100px;border-right:solid 1px #d5d5d5;">已提交的表单
	...
	<table class="table table-striped table-bordered table-hover"  style="width: 100%" >
			<c:forEach var="znNames" items="${znNames}">
         	 	...
         	 	<tr >    
	               		...
	               		<th scope="col" <c:if test="${khxm.hold==1}">style="padding-top: 0px !important; padding: 0px !important;text-align: center;background-position: right; background-repeat: no-repeat; background-image: url(/styles/images/application_edit.png);" </c:if> onclick="ajaxSubmit(this,'zznb_check','${khxm.id}','${khxm.hold }');">${khxm.zznb_check}</th> 
	...
	<a href="#" rel="tooltip"  data-placement="right"  data-original-title="${list.description}" ><i class=' icon-bell' ></i></a>
										...
										<td onclick="ajaxSubmit(this,'jzCount','${list.id}','${list.hold }');"<c:if test="${list.hold==1}">style="padding-top: 0px !important; padding: 0px !important;text-align: center;background-position: right; background-repeat: no-repeat; background-image: url(/styles/images/application_edit.png);"</c:if>>${list.jzCount}</td>
			...
			<c:if test="${flag != null && flag eq 2}">
  					<tr >
						...
						<td colspan="6" >
       						...
       						<span>
								<button  class="btn btn-primary save_submit" onclick="savekp()"><i class="icon-ok-sign"></i> <span class="save">保存</span></button>
								</span>								
	...
	function cx(){
        ...
        var orgid=document.getElementById("orgid").value;
         location.href="/kpxt/item/findjl?selectDate="+date+"&orgid="+orgid;
		}
	//区级相关复审
