...
<body>
    ...
    <li class="active">街镇助学结对统计表（单位版）</li><!--
    <li /kndyqk/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong> 街镇助学结对统计表（单位版）
  </div>
				<!--add end of-->
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="zxtjbdwb:add">
						<a  style="margin-bottom: 5px" href="/zxtjbdwb/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="zxtjbdwb:update">
											...
											<a class="btn " href="/zxtjbdwb/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="zxtjbdwb:delete">
											<a class="btn  b_delete_" href="/zxtjbdwb/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/zxtjbdwb/item/${businessForm.id }"><i class=" icon-folder-open"></i>名单</a>
										<shiro:hasPermission name="zxtjbdwb:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			<a class="btn b_submit_check" href="/zxtjbdwb/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
