...
<body>
	<ul class="breadcrumb" style="margin-bottom: 5px;">
	    ...
	    <li class="active">辖区内小区平面图</li>
  </ul>
			
	<div class="mianpan">
		  <div class="panhead">
		  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong>辖区内小区平面图
		  </div>
				<!--add end of-->
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="pmt:add">
						<a  style="margin-bottom: 5px" href="/pmt/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
					...
					<a style="margin-bottom: 5px"  class="btn btn-info" href="/taizhang/list/1/${userRoleCode}"><i class="icon-share-alt"></i> 返回</a>
							...
							<tbody>
								...
								<tr class="bgColor">
									...
									<a class="btn " href="/pmt/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
									<shiro:hasPermission name="pmt:delete">
											<a class="btn  b_delete_" href="/pmt/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> 
									<shiro:hasPermission name="pmt:update">
											
											
											<a class="btn " href="/pmt/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
...
<jsp:include page="/page/footer.jsp"></jsp:include>
	<script>
		$(function() {
			//分页
			var pagination = new buguPagination('${current}','${pages}', '${count}', '/pmt/list/');
