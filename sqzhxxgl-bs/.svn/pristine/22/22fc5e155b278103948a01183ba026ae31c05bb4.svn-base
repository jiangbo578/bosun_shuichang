...
<body>
    ...
    <li /tsjbzx/list/1 class="active">投诉举报咨询台账目录</li>-->
			...
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i> 红十字服务站物资登记册
			...
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="hszfwzwz:add">
						<a  style="margin-bottom: 5px" href="/hszfwzwz/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="hszfwzwz:update">
											...
											<a class="btn " href="/hszfwzwz/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="hszfwzwz:delete">
											<a class="btn  b_delete_zcdygk" href="/hszfwzwz/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/hszfwzwz/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
										<shiro:hasPermission name="hszfwzwz:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			                              <a class="btn b_submit_xfqcwh" href="/hszfwzwz/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
