...
<body>
    ...
    <li class="active">低保对象参加公益性社区服务劳动情况季度登记表</li><!--
    <li /tsjbzx/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i>低保对象参加公益性社区服务劳动情况季度登记表
			...
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="dbdxcjnd:add">
						<a  style="margin-bottom: 5px" href="/dbdxcjnd/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="dbdxcjnd:update">
											...
											<a class="btn " href="/dbdxcjnd/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="dbdxcjnd:delete">
											<a class="btn  b_delete_zcdygk" href="/dbdxcjnd/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/dbdxcjnd/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
										<shiro:hasPermission name="dbdxcjnd:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			                              <a class="btn b_submit_dbdxcjnd" href="/dbdxcjnd/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
...
<jsp:include page="/page/footer.jsp"></jsp:include>
	<script>
		$(function() {
			...
			$('.b_submit_dbdxcjnd').on('click', function() {
