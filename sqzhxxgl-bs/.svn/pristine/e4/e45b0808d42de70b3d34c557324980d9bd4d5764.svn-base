...
<body>
    ...
    <li class="active">居住在本居民区的在职党员概况名册</li><!--
    <li /zzdygk/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong> 居住在本居民区的在职党员概况名册
  </div>
				<!--add end of-->
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="zzdygk:add">
						<a  style="margin-bottom: 5px" href="/zzdygk/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="zzdygk:update">
											...
											<a class="btn " href="/zzdygk/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="zzdygk:delete">
											<a class="btn  b_delete_" href="/zzdygk/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/zzdygk/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
										<shiro:hasPermission name="zzdygk:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			<a class="btn b_submit_check" href="/zzdygk/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
