...
<body>
	<ul class="breadcrumb" style="margin-bottom: 5px;">
	    ...
	    <li class="active">共建、创建、联建单位名单</li>
  </ul>
			
	<div class="mianpan">
		  <div class="panhead">
		  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong>共建、创建、联建单位名单
		  </div>
				<!--add end of-->
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="gcldw:add">
						<a  style="margin-bottom: 5px" href="/gcldw/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
							...
							<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="gcldw:update">
											...
											<a class="btn " href="/gcldw/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="gcldw:delete">
											<a class="btn  b_delete_" href="/gcldw/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/gcldw/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
										<shiro:hasPermission name="gcldw:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			<a class="btn b_submit_check" href="/gcldw/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
