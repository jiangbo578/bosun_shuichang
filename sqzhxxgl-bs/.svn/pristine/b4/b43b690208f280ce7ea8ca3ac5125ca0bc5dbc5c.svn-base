...
<body>
    ...
    <li class="active">街镇助学结对统计表（个人版）</li><!--
    <li /kndyqk/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong> 街镇助学结对统计表（个人版）
  </div>
				<!--add end of-->
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="zxtjbgrb:add">
						<a  style="margin-bottom: 5px" href="/zxtjbgrb/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="zxtjbgrb:update">
											...
											<a class="btn " href="/zxtjbgrb/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="zxtjbgrb:delete">
											<a class="btn  b_delete_" href="/zxtjbgrb/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/zxtjbgrb/item/${businessForm.id }"><i class=" icon-folder-open"></i>名单</a>
										<shiro:hasPermission name="zxtjbgrb:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			<a class="btn b_submit_check" href="/zxtjbgrb/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
