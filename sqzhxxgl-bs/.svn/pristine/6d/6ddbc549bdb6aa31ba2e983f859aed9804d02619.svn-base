...
<body>
    ...
    <li class="active">嘉定区镇村市容环卫情况调查表</li><!--
    <li /tsjbzx/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i>嘉定区镇村市容环卫情况调查表
			...
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="srhwqk:add">
						<a  style="margin-bottom: 5px" href="/srhwqk/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="srhwqk:update">
											...
											<a class="btn " href="/srhwqk/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="srhwqk:delete">
											<a class="btn  b_delete_zcdygk" href="/srhwqk/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/srhwqk/item/${businessForm.id }"><i class=" icon-folder-open"></i>名单</a>
										<shiro:hasPermission name="srhwqk:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			                              <a class="btn b_submit_srhwqk" href="/srhwqk/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
...
<jsp:include page="/page/footer.jsp"></jsp:include>
	<script>
		$(function() {
			...
			$('.b_submit_srhwqk').on('click', function() {
