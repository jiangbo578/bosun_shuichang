...
<body>
    ...
    <li class="active">结对包户联系登记表</li><!--
    <li /jdbhlx/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong> 结对包户联系登记表
  </div>
				<!--add end of-->
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="jdbhlx:add">
						<a  style="margin-bottom: 5px" href="/jdbhlx/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="jdbhlx:update">
											...
											<a class="btn " href="/jdbhlx/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="jdbhlx:delete">
											<a class="btn  b_delete_" href="/jdbhlx/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/jdbhlx/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
										<shiro:hasPermission name="jdbhlx:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			<a class="btn b_submit_check" href="/jdbhlx/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
