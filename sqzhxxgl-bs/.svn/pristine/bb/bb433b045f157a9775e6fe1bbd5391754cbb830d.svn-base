...
<body>
    ...
    <li class="active">年度发展对象</li><!--
    <li /ndfzdx/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong> 年度发展对象
  </div>
				<!--add end of-->
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="ndfzdx:add">
						<a  style="margin-bottom: 5px" href="/ndfzdx/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="ndfzdx:update">
											...
											<a class="btn " href="/ndfzdx/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="ndfzdx:delete">
											<a class="btn  b_delete_" href="/ndfzdx/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/ndfzdx/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
										<shiro:hasPermission name="ndfzdx:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			<a class="btn b_submit_check" href="/ndfzdx/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
