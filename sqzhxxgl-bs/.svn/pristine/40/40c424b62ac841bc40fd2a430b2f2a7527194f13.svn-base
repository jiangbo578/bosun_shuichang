...
<body>
	<ul class="breadcrumb" style="margin-bottom: 5px;">
	    ...
	    <li class="active">公益性社区服务劳动参加对象汇总表</li>
  </ul>
			
	<div class="mianpan">
		  <div class="panhead">
		  <i class="icon-list">&nbsp;&nbsp;</i><strong>数据列表</strong>公益性社区服务劳动参加对象汇总表
		  </div>
				<!--add end of-->
			<div class="container" >
				<!--place end of-->
			
					<shiro:hasPermission name="fwnd:add">
						<a  style="margin-bottom: 5px" href="/fwnd/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
					...
					<a style="margin-bottom: 5px"  class="btn btn-info" href="/taizhang/list/3/${userRoleCode}"><i class="icon-share-alt"></i> 返回</a>
							...
							<tbody>
								...
								<tr class="bgColor">
									...
									<td>
									<div class="btn-group">
									<shiro:hasPermission name="fwnd:update">
											
											
											<a class="btn " href="/fwnd/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="fwnd:delete">
											<a class="btn  b_delete_" href="/fwnd/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/fwnd/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
										<shiro:hasPermission name="fwnd:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			<a class="btn b_submit_check" href="/fwnd/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
