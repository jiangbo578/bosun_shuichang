...
<body>
    ...
    <li class="active">不规范生猪养殖场（户）整治推进情况统计</li><!--
    <li /tsjbzx/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
			
			<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i>不规范生猪养殖场（户）整治推进情况统计
			...
			<div class="container" >
				<!--place end of-->
					...
					<shiro:hasPermission name="bgfyzc:add">
						<a  style="margin-bottom: 5px" href="/bgfyzc/add" class="btn btn-success"><i class="icon-plus"></i>新增表单</a>
						...
						<tbody>
								...
								<tr class="bgColor">
									...
									<shiro:hasPermission name="bgfyzc:update">
											...
											<a class="btn " href="/bgfyzc/update/${businessForm.id }"><i class="icon-pencil"></i>修改</a>
											
										</shiro:hasPermission> <shiro:hasPermission name="bgfyzc:delete">
											<a class="btn  b_delete_zcdygk" href="/bgfyzc/delete/${businessForm.id }"
												><i class="icon-trash"></i>删除</a>
										
										</shiro:hasPermission> <a class="btn " href="/bgfyzc/item/${businessForm.id }"><i class=" icon-folder-open"></i>查看与编辑</a>
										<shiro:hasPermission name="bgfyzc:submit">
										<c:if test="${businessForm.processStatus != '书记审核'}">
			                              <a class="btn b_submit_bgfyzc" href="/bgfyzc/submit/${businessForm.id}"><i class="icon-upload"></i>提交审核</a>
...
<jsp:include page="/page/footer.jsp"></jsp:include>
	<script>
		$(function() {
			...
			$('.b_submit_bgfyzc').on('click', function() {
