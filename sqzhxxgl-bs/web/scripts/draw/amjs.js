...
AmCharts.deleteObject = function(a, b) {
	if (a) {
		...
		if (0 != b) if ("[object Array]" === Object.prototype.toString.call(a)) for (var d = 0; d < a.length; d++) AmCharts.deleteObject(a[d], b - 1), a[d] = null;
		else try {
...
AmCharts.AmLegend = AmCharts.Class({
	...
	drawLegend: function() {
		...
		this.titleWidth = 0;
		if (b = this.title) a = AmCharts.text(this.container, b, this.color, a.fontFamily, this.fontSize, "start", !0), a.translate(0, this.marginTop + this.verticalGap + this.ly + 1), a = a.getBBox(), this.titleWidth = a.width + 15, this.titleHeight = a.height + 6;
...
AmCharts.AmRectangularChart = AmCharts.Class({
	...
	measureMargins: function() {
		...
		b = g - d;
		var l;
		for (l = 0; l < a.length; l++) b = this.getAxisBounds(a[l], h, k, j, b), h = b.l, k = b.r, j = b.t, b = b.b;
...
AmCharts.ValueAxis = AmCharts.Class({
	...
	draw: function() {
		...
		if (this.logarithmic && (0 >= this.getMin(0, this.data.length - 1) || 0 >= this.minimum)) this.fire("logarithmicAxisFailed", {
			...
			if (!isNaN(this.min) && !isNaN(this.max) && this.foundGraphs && Infinity != this.min && -Infinity != this.max) {
				...
				v = !1;
				for (e = p; e < r; e += s) t = AmCharts.roundTo(this.step * e + this.min, d), -1 != String(t).indexOf("e") && (v = !0, String(t).split("e"));
				this.duration && (this.maxInterval = AmCharts.getMaxInterval(this.max, this.duration));
				for (e = p; e < r; e += s) if (p = this.step * e + this.min, p = AmCharts.roundTo(p, this.maxDecCount + 1), !(this.integersOnly && Math.round(p) != p)) {
	...
	stackGraphs: function(a) {
		...
		if (p && ("regular" == b || "100%" == b)) for (n = 0; n < j.length; n++) m = j[n], m.hidden || (l = m.type, m.chart == this.chart && (m.valueAxis == this && a == l && m.stackable) && (k && (m.stackGraph = k), k = m));
...
AmCharts.CategoryAxis = AmCharts.Class({
	...
	draw: function() {
		...
		if (AmCharts.ifArray(a)) {
			...
			for (b = 0; b < j; b++) k = this.guides[b], p = p = p = h = t = NaN, e = k.above, k.toCategory && (p = d.getCategoryIndexByValue(k.toCategory), isNaN(p) || (t = this.getCoordinate(p - g), l = new this.axisItemRenderer(this, t, "", !0, NaN, NaN, k), this.pushAxisItem(l, e))), k.category && (p = d.getCategoryIndexByValue(k.category), isNaN(p) || (h = this.getCoordinate(p - g), p = (t - h) / 2, l = new this.axisItemRenderer(this, h, k.label, !0, NaN, p, k), this.pushAxisItem(l, e))), k.toDate && (this.equalSpacing ? (p = d.getClosestIndex(this.data, "time", k.toDate.getTime(), !1, 0, this.data.length - 1), isNaN(p) || (t = this.getCoordinate(p - g))) : t = (k.toDate.getTime() - this.startTime) * this.stepWidth, l = new this.axisItemRenderer(this, t, "", !0, NaN, NaN, k), this.pushAxisItem(l, e)), k.date && (this.equalSpacing ? (p = d.getClosestIndex(this.data, "time", k.date.getTime(), !1, 0, this.data.length - 1), isNaN(p) || (h = this.getCoordinate(p - g))) : h = (k.date.getTime() - this.startTime) * this.stepWidth, p = (t - h) / 2, l = "H" == this.orientation ? new this.axisItemRenderer(this, h, k.label, !1, 2 * p, NaN, k) : new this.axisItemRenderer(this, h, k.label, !1, NaN, p, k), this.pushAxisItem(l, e)), t = new this.guideFillRenderer(this, h, t, k), h = t.graphics(), this.pushAxisItem(t, e), k.graphics = h, h.index = b, k.balloonText && this.addEventListeners(h, k)
		}
		this.axisCreated = !0;
		d = this.x;
		g = this.y;
...
AmCharts.RadItem = AmCharts.Class({
	construct: function(a, b, d, e, f, g, h) {
		...
		if (0 < r) {
			if ("polygons" == z) {
				C = a.data.length;
				for (k = 0; k < C; k++) l = 180 - 360 / C * k, d.push(b * Math.sin(l / 180 * Math.PI)), j.push(b * Math.cos(l / 180 * Math.PI));
				d.push(d[0]);
				j.push(j[0]);
...
AmCharts.AmGraph = AmCharts.Class({
	...
	drawSmoothedGraph: function(a, b, d, e) {
		if (1 < a.length) {
			...
			if (0 < m && (l = a.join(";").split(";"), u = b.join(";").split(";"), k = "", 0 < d.length ? (d.reverse(), e.reverse(), l = a.concat(d), u = b.concat(e)) : (this.rotate ? (k += " L" + q + "," + b[b.length - 1], k += " L" + q + "," + b[0]) : (k += " L" + a[a.length - 1] + "," + q, k += " L" + a[0] + "," + q), k += " L" + a[0] + "," + b[0]), d = new AmCharts.Bezier(g, l, u, NaN, 0, 0, n, m, f, k), h.push(d.path), p || void 0 !== s)) r || (r = m), p || (p = s), a = new AmCharts.Bezier(g, a, b, NaN, 0, 0, p, r, f, k), j.push(a.path);
			this.applyMask(j, h)
		}
	},
				...
				if (d.mouseIsOver || z || C || this.forceShow) if (this.forceShow = !1, C) {
					...
					if (I && this.valueBalloonsEnabled && v && d.balloon.enabled) {
						...
						if (this.oneBalloonOnly) {
							j = Infinity;
							for (g = 0; g < I.length; g++) u = I[g], u.showBalloon && (!u.hidden && u.balloonText) && (t = s.axes[u.valueAxis.id].graphs[u.id], w = t.y, isNaN(w) || (r ? Math.abs(e - w) < j && (j = Math.abs(e - w), O = u) : Math.abs(f - w) < j && (j = Math.abs(f - w), O = u)));
	...
	dispatchScrollbarEvent: function() {
		if (this.skipEvent) this.skipEvent = !1;
			...
			if (g.parseDates && !g.equalSpacing) {
				if (a = e.firstTime, g.minDuration(), e = Math.round(b / h) + a, a = this.dragging ? e + this.timeDifference : Math.round((b + d) / h) + a, e > a && (e = a), e != this.startTime || a != this.endTime) this.startTime = e, this.endTime = a, f.start = e, f.end = a, f.startDate = new Date(e), f.endDate = new Date(a), this.fire(f.type, f)
			} else if (g.startOnAxis || (b += h / 2), d -= this.stepWidth / 2, e = g.xToIndex(b), b = g.xToIndex(b + d), e != this.start || this.end != b) g.startOnAxis && (this.resizingRight && e == b && b++, this.resizingLeft && e == b && (0 < e ? e-- : b = 1)), this.start = e, this.end = this.dragging ? this.start + this.difference : b, f.start = this.start, f.end = this.end, g.parseDates && (a[this.start] && (f.startDate = new Date(a[this.start].time)), a[this.end] && (f.endDate = new Date(a[this.end].time))), this.fire(f.type, f)
		}
	},
	zoomScrollbar: function() {
		var a, b;
