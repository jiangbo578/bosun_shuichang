...
<body>
    ...
    <li class="active">已提交的600分评级</li>
  </ul>
   <div class="panhead"><i class="icon-pencil"></i>&nbsp;&nbsp;<strong>已提交的600分评级</strong></div>
   	<div class="mianpan">
	  <div id="panbody">
		<div class="container">
  	<div class="tabbox" >
  <ul class="tabbtn">
  			<c:forEach var="assess" items="${assess}">
				...
				<span style="width: 100px;border-right:solid 1px #d5d5d5;" onclick="submitByBusiness('/kpxt/item/submit?assessId=${assess.id}');">${assess.name }
				</span>
			</li>
  			</c:forEach>
		</ul>
					...
					<c:forEach items="${list}" var="list" varStatus="num">
						...
						<c:if test="${khxm.mbid eq list.assessTemplate.id}">
									<tr>
										...
										<td <c:if test="${list.hold==0}">style="background: #F0F0F0"</c:if>>${list.count}</td><!-- 截止今年末 -->
										...
										<%--	
										<td><input name="assesses[${num.count-1 }].jzjnsbn" type="text" value="${list.jzjnsbn }" data-rule="required:digits;" style="width: 80px;"/></td>
										<td><input name="assesses[${num.count-1 }].jzjnend" type="text" value="${list.jzjnend }" data-rule="required:digits;" style="width: 80px;"/></td>
										--%>
									</tr>
						</c:if>
					</c:forEach>
