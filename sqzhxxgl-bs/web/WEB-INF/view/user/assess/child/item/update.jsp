...
<body>
 ...
 <li><a href="/assess/child/list/${assessId }/1">${assess.name}[${assess.identityName }]-考核项目</a> <span class="divider">/</span></li>
  <li class="active">【${template.assessTemplate.project }】-考核类别</li>
    <!--
    <li /tsjbzx/list/1 class="active">投诉举报咨询台账目录</li>-->
  </ul>
    	<div class="mianpan">
  <div class="panhead">
  <i class="icon-list">&nbsp;&nbsp;</i><strong>修改考核类别</strong> 
  
   
  </div>
 
	<div class="container">
		<form action="/assess/child/item/update2/${assessId}/${proId}" method="post">

			<table>
				<tr>
					<th>考核项目名称：</th>
					<td>${template.assessTemplate.project }</td>
				</tr>
				<tr>
					<th>考核类别名称：</th>
					<td><input name="type" type="text" data-rule="required;" style="width: 700px" value="${template.type }"/>
					</td>
				</tr>
				<tr>
					<th>考核规则类型：</th>
					<td>
						...
						<input type="radio" name="khgz"  value="无证土地">无证土地
						<input type="radio" name="khgz"  value="支配">支配
					</td>
				</tr>
				<tr>
					<th>扣分限值：</th>
					<td><input name="kfxz" type="text" style="width: 200px" value="${template.kfxz}"/>
					</td>
				</tr>
			<tr>
					<th>平均值：</th>
					<td><input name="vag" type="text" style="width: 200px" value="${template.vag }"/>请输入小数或者整数
					</td>
				</tr>
				<tr>
					<th>往年数据是否需要手动填报：</th>
				...
				<tr>
					<th>描述：</th>
					<td><input name="description" type="text"  style="width: 200px" value="${template.description }"/>
					</td>
				</tr>
				<tr>
					<th>&nbsp;</th>
	...
	<jsp:include page="/page/footer.jsp"></jsp:include><br>		<br><br>
		 ...
		 $(function(){
			 ...
			 $('input[name=khgz][type=radio][value=${template.khgz}]').attr('checked', true);	
			 $('input[name=sfxy][type=radio][value=${template.sfxy}]').attr('checked', true);	
	   		});
        parent.iFrameHeight();
